...
def _cuda_available() -> bool:
    """GPU(CUDA) 사용 가능 여부. torch가 없거나 CPU 빌드면 False."""
    try:
        import torch
...
def load_config() -> dict:
    ...
    # use_fp16은 GPU(CUDA)가 있을 때만 의미가 있으므로, CUDA가 없으면(예: CPU 빌드)
    # 아예 키를 만들지 않는다. CUDA가 있으면 기본 True.
    if _cuda_available():
        cfg.setdefault("use_fp16", True)
        _removed = False
    else:
        _removed = cfg.pop("use_fp16", None) is not None  # 남아 있던 키 제거
    ...
    # 첫 실행(파일 없음/깨짐)이거나, CPU 환경에서 옛 use_fp16 키를 제거한 경우 저장
    if not existed or _removed:
        try:
            save_config(cfg)
        except Exception:
            pass
