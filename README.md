...
| exp07 | 임베딩 + ArcFace | 각도 마진 손실. margin·scale 12조합 그리드 탐색 |
...
\* exp06·07의 FPS는 임베딩에서 분류 헤드만 통과하는 속도(백본 제외)라 다른 실험과 직접 비교 대상은 아님.

**exp07 — ArcFace margin×scale 12조합 그리드** (값은 test 정확도)
...
최고는 **margin 0.5 / scale 16에서 0.9723**(전체 최고). 다만 12조합 모두 0.9668~0.9723에 몰려(편차 0.55%p), 마진·스케일 선택에 성능이 크게 좌우되지 않았습니다. ArcFace 논문 권장값(margin 0.5) 부근이 가장 안정적이었습니다.
...
- **점진적으로 난이도를 올려 성능 상한을 확인했다.** 단순 유사도(exp00, 0.9601) → 분류(exp01·02·04) →
  직접 설계한 MLP(exp06, 0.9696) → 얼굴 인식 SOTA 기법 ArcFace 12조합 그리드(exp07, 최고 0.9723)까지
  올려봤다. 성능은 0.96→0.97로 미미하게 올랐을 뿐이며, 사전학습 기반 방식들의 차이는 **1%p 안팎으로
  유의미하다고 보기 어렵다**(테스트 표본 723장).
- **학습 없는 유사도 방식(exp00)도 사실상 동등하다.** 정확도 0.9601로 최고(exp07, 0.9723)와 1.2%p 차이이며,
  별도 학습이 전혀 필요 없다.

**최종 결론 — 실제 앱은 유사도(임베딩) 방식을 채택한다.**
ArcFace까지 동원해 성능 상한을 확인했지만 그 우위는 유의미하지 않고, 분류·MLP·ArcFace 방식은
**새 직원이 들어올 때마다 출력 클래스를 바꿔 재학습**해야 한다. 반면 유사도 방식은 사전학습 임베딩만으로
동작하므로 **재학습 없이 등록만으로 직원을 추가**할 수 있고 속도도 빠르다.
즉 **성능 상한(exp07)과 운영상 최적(exp00)을 모두 확인한 뒤**, 인원이 수시로 바뀌는 출퇴근 환경에서는
...
# 6) (추가) ArcFace margin/scale 그리드 -> exp07, runs/arcface_grid.csv
python train_arcface.py

# 7) (추가) 임베딩 분석 그림 생성 -> runs/figs/
