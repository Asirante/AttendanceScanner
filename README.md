...
- **분류 방식이 수치상 가장 높긴 했다.** 최고는 exp04(증강 끔, 0.9682)였다.
  다만 동결·미세조정·증강 유무 사이의 차이는 **0.1~1%p 수준으로 유의미하다고 보기 어렵다**
  (테스트 표본이 723장이라 몇 장 차이로 뒤바뀌는 정도). 미세조정(exp02)도 동결(exp01) 대비 이득이 없었다.
- **학습 없는 유사도 방식(exp00)이 분류 방식과 사실상 동등하다.** 정확도 0.9601로 exp01과의 차이는
  약 0.7%p에 불과하며, 속도는 오히려 2배 이상 빠르다(89 vs 42 FPS).

**최종 결론 — 실제 앱은 유사도(임베딩) 방식을 채택한다.**
분류 방식이 근소하게 높지만 그 차이는 유의미하지 않고, 분류 모델은 **새 직원이 들어올 때마다
출력 클래스를 바꿔 재학습**해야 한다. 반면 유사도 방식은 사전학습 임베딩만으로 동작하므로
**재학습 없이 등록만으로 직원을 추가**할 수 있고 속도도 빠르다.
출퇴근처럼 인원이 수시로 바뀌는 환경에서는 운영 편의성이 결정적이므로,
정확도가 거의 동일한 유사도 방식이 실용적으로 가장 적합하다.
